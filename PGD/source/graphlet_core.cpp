...
void graphlet_core::read_graph(const string& filename) {
	...
	// commented by Daniel Hammer
	// reason: unnecessary filling of command line
	//basic_stats();
}
...
inline void graphlet_core::load_progress_bar(long long x, long long n, long long w) {
 ...
 void graphlet_core::write_micro_stats(string &filename, bool output_id, unsigned int buffer_size) {
	...
	/*
	 	commented out by Daniel Hammer
	 	reason: not helpful for the files and would produce work later on (filtering out)
     ...
     if (buffer_size==0) {
    	 ...
    	 if (output_id) {
    		 for (long long e=0; e<E_ordered.size(); e++) {
				 ...
				 // Edited: swap the order of the nodes to small -> big (Daniel Hammer)
    			 long long v_ = e_v[edge_id], u_ = e_u[edge_id];
				 long long v = min(u_, v_);
				 long long u = max(u_, v_);
    			 long long deg_v = vertices[v+1]-vertices[v], deg_u = vertices[u+1]-vertices[u];
