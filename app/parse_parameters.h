...
// ml practical
        ...
        struct arg_lit *train                                = arg_lit0(NULL, "train", "Train the 'booster.json' of the current directory on the specified data (creates one if not existing)");
        struct arg_lit *labelPropagation                     = arg_lit0(NULL, "labelPropagation", "Perform a label propagation using the weights predicted by the ml model 'booster.json' of the current directory.");
        struct arg_lit *check                                = arg_lit0(NULL, "check", "add to labelPropagation with a dataFile that contains the correct labeling");
        struct arg_lit *random                               = arg_lit0(NULL, "random", "use random weights in [0, 1] for the labelPropagation");

        struct arg_str *graphFilename                        = arg_strn(NULL, "graphFile", "graphFilename", 0, 1, "Path to graph file to work on.");
        struct arg_str *outFilename                          = arg_strn(NULL, "outFilename", "outFilename", 0, 1, "Specify the file in which the data is written.");
        struct arg_str *communityFilename                    = arg_strn(NULL, "communityFilename", "communityFilename", 0, 1, "Path to graphs community file to work with.");
        struct arg_str *graphletFilename                     = arg_strn(NULL, "graphletFilename", "graphletFilename", 0, 1, "Path to graphs graphlet file to work with (if already existing).");

        struct arg_str *dataFilename                         = arg_strn(NULL, "dataFilename", "dataFilename", 0, 1, "Path to graphs data file to learn/predict from.");
#endif
...
#elif defined MODE_ML_CREATE_DATA
                communityFilename,
                graphletFilename,
                featureFilename,
...
#ifdef MODE_MLCLUSTERING
        partition_config.createData = createData->count;
        partition_config.train = train->count;
        partition_config.labelPropagation = labelPropagation->count;
        partition_config.check = check->count;
        partition_config.random = random->count;

        partition_config.graphFilename     = graphFilename->sval[0];
        partition_config.outFilename       = outFilename->sval[0];
        partition_config.communityFilename = communityFilename->sval[0];
        partition_config.graphletFilename  = graphletFilename->sval[0];
        partition_config.dataFilename  = dataFilename->sval[0];
#endif
